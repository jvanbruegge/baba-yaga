...
public class SSpawnMyObjectPacket extends SSpawnGlobalEntityPacket implements IPacket<IClientPlayNetHandler>  {
	...
	public SSpawnMyObjectPacket(int entityID, UUID uuid, double posX, double posY, double posZ, float pitch, float yaw, EntityType<?> type, int data, Vec3d velocity) {
		   ...
		   public SSpawnMyObjectPacket(Entity entity) {
		      this(entity, 0);
		   }

		   public SSpawnMyObjectPacket(Entity entityIn, int typeIn) {
		      this(entityIn.getEntityId(), entityIn.getUniqueID(),
		    		  entityIn.posX, entityIn.posY, entityIn.posZ, 
		    		  entityIn.rotationPitch, entityIn.rotationYaw, 
		    		  entityIn.getType(), typeIn, entityIn.getMotion());
		   }

		   public SSpawnMyObjectPacket(Entity entity, EntityType<?> type, int data, BlockPos blockPos) {
		      this(entity.getEntityId(), entity.getUniqueID(), 
		    		  (double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ(), 
		    		  entity.rotationPitch, entity.rotationYaw, type, data, entity.getMotion());
		   }
